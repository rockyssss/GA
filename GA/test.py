...
def draw_new(st_x, st_y, Z, px, py, pz):
    plt.cla()
    # ax = Axes3D(fig)
...
def hill_climb(X, Y):
    ...
    while (temperature > tmin):
        ...
        i = i+1
        z = func(st_x, st_y)
        print(i,  'x:', st_x,  'y:', st_y, 'z:', z, 'temperature:', temperature)
        Z = func(X, Y, 1.7, 1.7)
        px = global_X
        py = global_Y
        # if st_x != global_X[len(global_X)-2]:
        #     drawPaht(X, Y, Z, [st_x], [st_y], [z])
        # drawPaht(X, Y, Z, [st_x], [st_y], [z])
        # plt.pause(0.0001)
        # plt.show()
...
if __name__ == '__main__':
    X = np.arange(-2, 4, 0.1)  # 产生，[-2, 4]之间，步长是0.1的np一维数组
    Y = np.arange(-2, 4, 0.1)  # 产生，[-2, 4]之间，步长是0.1的np一维数组
    X, Y = np.meshgrid(X, Y)
    Z = func(X, Y, 1.7, 1.7)
